...
public:
	...
	RTFArchive* CreateFileReader(const char* Filename, uint32_t ReadFlags = 0) override { return nullptr; }
	RTFArchive* CreateFileWriter( const char* Filename, uint32_t WriteFlags=0 ) override { return nullptr; };
	...
	bool IsReadOnly( const char* Filename ) override { return false; }
	bool Delete( const char* Filename, bool RequireExists=0, bool EvenReadOnly=0, bool Quiet=0 ) override { return false; }
	uint32_t Copy( const char* Dest, const char* Src, bool Replace=1, bool EvenIfReadOnly=0, bool Attributes=0, ECopyResult* Progress = nullptr, EFileRead ReadFlags=FILEREAD_None, EFileWrite WriteFlags=FILEWRITE_None) override { return false; }
	bool Move( const char* Dest, const char* Src, bool Replace=1, bool EvenIfReadOnly=0, bool Attributes=0, bool bDoNotRetryOrError=0 ) override { return false; }
	bool FileExists( const char* Filename ) override { return false; }
	bool DirectoryExists( const char* InDirectory )override { return false; }
	bool MakeDirectory( const char* Path, bool Tree=0 )override { return false; }
	bool DeleteDirectory( const char* Path, bool RequireExists=0, bool Tree=0 )override { return false; }
...
private:
	RTFArchive* CreateFileReaderInternal(const char* Filename, uint32_t ReadFlags, uint32_t BufferSize) override { return nullptr; }
	RTFArchive* CreateFileWriterInternal(const char* Filename, uint32_t WriteFlags, uint32_t BufferSize) override { return nullptr; }
	uint32_t CopyWithProgress(const char* InDestFile, const char* InSrcFile, bool ReplaceExisting, bool EvenIfReadOnly, bool Attributes, ECopyResult* Progress, EFileRead ReadFlags, EFileWrite WriteFlags) override { return 0; }
