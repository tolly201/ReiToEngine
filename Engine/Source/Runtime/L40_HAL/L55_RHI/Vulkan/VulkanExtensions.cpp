...
void get_vulkan_extensions(u32& out_extension_count, ReiToEngine::List<const char*>& out_extensions)
{
    ...
    RT_LOG_INFO_PLATFORM("Vulkan Extension: ");
    for(auto& ext : out_extensions) {
        RT_LOG_INFO_PLATFORM(ext);
    }
}
...
b8 get_vulkan_validation_layers(u32& out_layer_count, ReiToEngine::List<const char*>& out_layers)
{
    ...
    out_layers.clear();
#if RT_OHI_DEBUG == 1
    RT_LOG_INFO_PLATFORM("Vulkan Validation Layers enabled.");
    ...
    for (u32 i = 0; i < out_layers.size(); ++i) {
        RT_LOG_INFO_PLATFORM("Requested Vulkan Validation Layer: ", out_layers[i]);
        ...
        if (!layer_found) {
            RT_LOG_FATAL_PLATFORM("Vulkan Validation Layer not found: ", out_layers[i]);
            out_layers.clear();
            out_layer_count = 0;
            return false;
        }
    ...
    RT_LOG_INFO_PLATFORM("All requested Vulkan Validation Layers are available.");
...
b8 create_debugger(VkInstance& instance, VkAllocationCallbacks*& allocator, VkDebugUtilsMessengerEXT& debug_messenger)
{
    ...
    RT_ASSERT_MESSAGE_PLATFORM(func != nullptr, "Failed to get vkCreateDebugUtilsMessengerEXT function.");
    RT_VK_CHECK(func(instance, &debug_create_info, allocator, &debug_messenger));
    RT_LOG_DEBUG_PLATFORM("Vulkan Debug Messenger created.");
    return true;
#endif
    ...
    [[maybe_unused]]void*                                        pUserData)
{
    if (messageSeverity & VK_DEBUG_UTILS_MESSAGE_SEVERITY_ERROR_BIT_EXT) {
        RT_LOG_ERROR_PLATFORM("Vulkan Validation Layer: ", pCallbackData->pMessage);
    } else if (messageSeverity & VK_DEBUG_UTILS_MESSAGE_SEVERITY_WARNING_BIT_EXT) {
        RT_LOG_WARN_PLATFORM("Vulkan Validation Layer: ", pCallbackData->pMessage);
    } else if (messageSeverity & VK_DEBUG_UTILS_MESSAGE_SEVERITY_INFO_BIT_EXT) {
        RT_LOG_INFO_PLATFORM("Vulkan Validation Layer: ", pCallbackData->pMessage);
    } else if (messageSeverity & VK_DEBUG_UTILS_MESSAGE_SEVERITY_VERBOSE_BIT_EXT)
    {
        RT_LOG_TRACE_PLATFORM("Vulkan Validation Layer: ", pCallbackData->pMessage);
    }
    return VK_FALSE;
}
}
