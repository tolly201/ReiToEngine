...
#include "../L23_MemoryManager/Memory.h"
   ...
   public:
    ...
    static MemoryManager& GetMemoryManager()
    {
        if (!memoryManager.IsSeted())
        {
            ...
            RT_LOG_WARN_PLATFORM("Memory manager not set. Using default Mimalloc memory manager.");
        }
        return memoryManager;
    };
    static b8 SetMemoryManager(void* MM = nullptr, EMEMORY_MANAGER_TYPE type = EMEMORY_MANAGER_TYPE::MIMALLOC)
    {
        RT_ASSERT_MESSAGE_PLATFORM(type != EMEMORY_MANAGER_TYPE::DEFAULT, "EMEMORY_MANAGER_TYPE::DEFAULT is not a valid type for setting memory manager. Use a specific type like MIMALLOC, STANDARDC, BINNED, or CUSTOM.");

        if (type == EMEMORY_MANAGER_TYPE::CUSTOM) {
            RT_ASSERT_MESSAGE_PLATFORM(MM != nullptr, "When setting a custom memory manager, MM must not be nullptr.");
        } else {
            RT_ASSERT_MESSAGE_PLATFORM(MM == nullptr, "When not setting a custom memory manager, MM must be nullptr.");
        }

        RT_ASSERT_MESSAGE_PLATFORM(!memoryManager.IsSeted(), "Memory manager can only be set once.");
...
class Singleton {
   ...
   public:
    ...
    static T& Instance() {
        RT_ASSERT_PLATFORM(instance_ptr != nullptr && "Singleton instance not initialized. Call InitializeInstance() first if using IndependentConstruct.");
        return *instance_ptr;
    }
