...
b8 EventSystem::Initialize(){
    if (is_initialized) {
        RT_LOG_ERROR_PLATFORM("EventSystem is already initialized.");
        return false;
    }
    ...
    is_initialized = true;
    RT_LOG_INFO_PLATFORM("EventSystem initialized successfully.");
    return true;
}
...
b8 EventSystem::RegisterEvent(u32 code, void* listener, PFN_on_event callback){
    if (!is_initialized) {
        RT_LOG_ERROR_PLATFORM("EventSystem is not initialized.");
        return false;
    }
