...
namespace ReiToEngine
{
            ...
            if (handle)
            {
                switch (type)
                {
                    ...
                    case EMEMORY_MANAGER_TYPE::CUSTOM:
                        ...
                        break;
                    default:
                        RT_LOG_WARN_PLATFORM("Unknown memory manager type in MemoryManager destructor.");
                        break;
                }
                handle = nullptr;
            }
    ...
    {
        RT_ASSERT_MESSAGE_PLATFORM(!is_set, "Memory manager can only be set once.");
    ...
    rt_spin_lock(&lock_state);
        ...
        RT_LOG_WARN_FMT_PLATFORM("Setting memory manager of type: {}", static_cast<void*>(newWrapper));
        ...
        else
        {
            switch (t)
            {
                ...
                case EMEMORY_MANAGER_TYPE::BINNED:
                    ...
                    break;
                case EMEMORY_MANAGER_TYPE::CUSTOM:
                    RT_LOG_ERROR_PLATFORM("CUSTOM backend requires external instance passed via MM (ICustomMemoryManager*).");
                    ...
                    return false;
                default:
                    RT_LOG_ERROR_PLATFORM("Unknown EMEMORY_MANAGER_TYPE specified in MemoryManager::SetMemoryManager. Fallback to Mimalloc.");
            ...
            RT_LOG_WARN_FMT_PLATFORM("Create memory manager of type: {}", static_cast<void*>(newWrapper->handle));
            return true;
        }
        rt_spin_unlock(&lock_state);
    ...
    {
        RT_ASSERT_MESSAGE_PLATFORM(wrappers, "MemoryManager not initialized in Allocate.");
    ...
    rt_spin_unlock(&lock_state);
        ...
        switch (type)
        {
            ...
            default:
                RT_ASSERT_MESSAGE_PLATFORM(false, "Unknown memory manager type in MemoryManager::Allocate.");
                return nullptr;
        }
        RT_ASSERT_MESSAGE_PLATFORM(raw != nullptr, "Memory allocation failed in MemoryManager::Allocate.");
        ...
        if (use_fixed) {
            ...
            RT_ASSERT_MESSAGE_PLATFORM((user_addr % user_alignment) == 0, "Fixed-path alignment failure (unexpected).\n");
            ...
            while (aligned_candidate + size > region_end) {
                ...
                if (next <= aligned_candidate || next + size > region_end) {
                    RT_ASSERT_MESSAGE_PLATFORM(false, "Failed to place aligned allocation with header inside reserved block (logic error - dynamic).");
                    return nullptr;
                }
                aligned_candidate = next;
            }
            uintptr_t header_addr = aligned_candidate - head_info_size;
            RT_ASSERT_MESSAGE_PLATFORM(header_addr >= raw_addr && header_addr + head_info_size <= region_end, "Header out of reserved range (dynamic).");
            MemoryManagerWrapperLayerHeadInfo* head = reinterpret_cast<MemoryManagerWrapperLayerHeadInfo*>(header_addr);
            head->allocator = handle;
            head->actual_offset = (u64)(header_addr - raw_addr);
            head->actual_size = request_size;
            void* user_ptr = reinterpret_cast<void*>(aligned_candidate);
            RT_ASSERT_MESSAGE_PLATFORM(((uintptr_t)user_ptr % user_alignment) == 0, "User pointer is not properly aligned after allocation (dynamic path).");
            return user_ptr;
        }
    }
    ...
    {
        if (!addr) return;
    RT_ASSERT_MESSAGE_PLATFORM(wrappers, "MemoryManager not initialized in Free.");
        ...
        RT_ASSERT_MESSAGE_PLATFORM(found, "Allocator not found in MemoryManager::Free. Possible double free or memory corruption.");
        ...
        switch (type)
        {
            ...
            default:
                RT_LOG_ERROR_PLATFORM("Unknown memory manager type in MemoryManager::Free. Falling back to Mimalloc.");
                return static_cast<RTMimallocManager*>(handle)->Free((void*)((uintptr_t)head - head->actual_offset), total_size, tag);
        }
    }
    void* MemoryManager::ZeroMemoryReiTo(void* addr, u64 size)
    {
    RT_ASSERT_MESSAGE_PLATFORM(wrappers, "MemoryManager not initialized in ZeroMemoryReiTo.");
    rt_spin_lock(&lock_state);
        void* handle = wrappers->handle;
        EMEMORY_MANAGER_TYPE type = wrappers->type;
    rt_spin_unlock(&lock_state);
        switch (type)
        {
            ...
            default:
                RT_LOG_ERROR_PLATFORM("Unknown memory manager type in MemoryManager::ZeroMemoryReiTo. Falling back to Mimalloc.");
                return static_cast<RTMimallocManager*>(handle)->ZeroMemoryReiTo(addr, size);
        }
    }
    void* MemoryManager::CopyMemoryReiTo(void* dest, const void* src, u64 size)
    {
    RT_ASSERT_MESSAGE_PLATFORM(wrappers, "MemoryManager not initialized in CopyMemoryReiTo.");
    rt_spin_lock(&lock_state);
        void* handle = wrappers->handle;
        EMEMORY_MANAGER_TYPE type = wrappers->type;
    rt_spin_unlock(&lock_state);
        switch(type)
        {
            ...
            default:
                RT_LOG_ERROR_PLATFORM("Unknown memory manager type in MemoryManager::CopyMemoryReiTo. Falling back to Mimalloc.");
                return static_cast<RTMimallocManager*>(handle)->CopyMemoryReiTo(dest, src, size);
        }
    }
    void* MemoryManager::SetMemory(void* addr, u8 value, u64 size)
    {
    RT_ASSERT_MESSAGE_PLATFORM(wrappers, "MemoryManager not initialized in SetMemory.");
    rt_spin_lock(&lock_state);
        void* handle = wrappers->handle;
        EMEMORY_MANAGER_TYPE type = wrappers->type;
    rt_spin_unlock(&lock_state);
        switch (type)
        {
            ...
            default:
                RT_LOG_ERROR_PLATFORM("Unknown memory manager type in MemoryManager::SetMemory. Falling back to Mimalloc.");
                return static_cast<RTMimallocManager*>(handle)->SetMemory(addr, value, size);
        }
    }
    char* MemoryManager::GetMemoryUsageReport(){
    RT_ASSERT_MESSAGE_PLATFORM(wrappers, "MemoryManager not initialized in GetMemoryUsageReport.");
    rt_spin_lock(&lock_state);
        void* handle = wrappers->handle;
        EMEMORY_MANAGER_TYPE type = wrappers->type;
    rt_spin_unlock(&lock_state);
        switch (type)
        {
            ...
            default:
                RT_LOG_ERROR_PLATFORM("Unknown memory manager type in MemoryManager::GetMemoryUsageReport. Falling back to Mimalloc.");
                return static_cast<RTMimallocManager*>(handle)->GetMemoryUsageReport();
        }
    }
}
