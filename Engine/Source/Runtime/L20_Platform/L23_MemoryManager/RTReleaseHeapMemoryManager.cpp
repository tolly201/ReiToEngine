#include "RTReleaseHeapMemoryManager.h"
...
RTReleaseHeapMemoryManager::RTReleaseHeapMemoryManager()
{
	SystemPageSize = 0 ;
	SystemPageSize = 64;
	RT_ASSERT_PLATFORM((SystemPageSize&(SystemPageSize-1)) == 0 && "SystemPageSize is not 2^n");
	...
	for( size_t i=0; i < POOL_MAX; i++ )
	{
		...
		RT_ASSERT_PLATFORM(Index < POOL_COUNT && "POOL MAX OVERSIZE.");
	...
	RT_ASSERT_PLATFORM(POOL_MAX - 1 == PoolTable[POOL_COUNT - 1].BlockSize && "Pool Max Size Error. Initialize Memory Pool Failed.");
}
	...
	for (unsigned int i = 0 ; i < 32 ; i++)
	{
		for (unsigned int j = 0 ; j < 2048 ; j++)
		{
			if (PoolIndirect[i])
			{
				...
				{
					RT_Platform_SYSFree( PoolIndirect[i][j].Mem, PoolIndirect[i][j].Bytes);
...
void* RTReleaseHeapMemoryManager::SetMemoryImpl(void* ptr, u8 val, u64 size) {
	RT_Platform_SYSSetMemory(ptr, val, size);
	return nullptr;
};
...
void* RTReleaseHeapMemoryManager::AllocateImpl(u64 uiSize,[[maybe_unused]] uint8_t u8,[[maybe_unused]] RT_MEMORY_TAG tag)
{
	...
	if( uiSize<POOL_MAX )
	{
		...
		RT_ASSERT_PLATFORM(uiSize<=Table->BlockSize && "Memory Size To Pool Check Error.");
		...
		if( !Pool )
		{
			...
			RT_ASSERT_PLATFORM(Blocks>=1 && "Blocks Counting Error.");
			RT_ASSERT_PLATFORM(Blocks*Table->BlockSize<=Bytes && "Bytes Counting Error.");
		...
		Pool->RefCount++;
		RT_ASSERT_PLATFORM(Pool->FirstMem && "allocation failed.");
		RT_ASSERT_PLATFORM(Pool->FirstMem->Blocks > 0 && "allocation failed.");
	...
	else
	{
		...
		Free = (FFreeMem*)RT_Platform_SYSAlloc(AlignedSize);
		if( !Free )
		{
			return nullptr;
		}
		RT_ASSERT_PLATFORM(!((size_t)Free&65535));
...
void RTReleaseHeapMemoryManager::FreeImpl(void* pcAddr,[[maybe_unused]] u64 size,[[maybe_unused]] RT_MEMORY_TAG tag)
{
	...
	if( Pool->Table!=&OsTable )
	{
		...
		// Free this pool.
		RT_ASSERT_PLATFORM(Pool->RefCount>=1);
		if( --Pool->RefCount == 0 )
		{
			// Free the OS memory.
			Pool->Unlink();
			RT_Platform_SYSFree( Pool->Mem, 0);
			Pool->Mem = NULL;
		}
	}
	else
