#include "RTDebugStackMemoryManager.h"
#include <iostream>
...
RTDebugStackMemoryManager::~RTDebugStackMemoryManager()
{
    ...
    while (currentTracked)
    {
        ...
        RT_Platform_SYSFree(currentTracked, currentTracked->Size + sizeof(RTBlock)); // 释放 RTBlock 结构和用户内存
        currentTracked = next;
    }
}
...
void* RTDebugStackMemoryManager::allocatePoolCall(uint32_t size,[[maybe_unused]] uint32_t alignment)
{
     if (size > defaultSmallTargetMemoryAlignment) // 内存池只处理固定大小的内存块
    {
        ...
        return nullptr; //  或者可以回退到 RT_Platform_SYSAlloc 分配
    }
...
void RTDebugStackMemoryManager::deAllocatePoolCall(void* addr,[[maybe_unused]] uint32_t alignment)
{
    ...
    // RT_Platform_SYSFree(block, block->Size + sizeof(RTBlock)); //  内存池模式下，不再释放回系统
}
...
void* RTDebugStackMemoryManager::allocateSystemCall(uint32_t size, uint32_t alignment)
{
    ...
    RTBlock* block = reinterpret_cast<RTBlock*>(RT_Platform_SYSAlloc(totalSize));
    if (!block)
    {
...
void RTDebugStackMemoryManager::deAllocateSystemCall(void* addr,[[maybe_unused]] uint32_t alignment)
{
    ...
    RT_Platform_SYSFree(block, block->Size + sizeof(RTBlock));
}
