...
RT_TEST(SMatrix_multiply_identity){
    SMatrix<3,3,float> I{1,0,0, 0,1,0, 0,0,1};
    SMatrix<3,3,float> A{1,2,3, 4,5,6, 7,8,9};
    auto L = I * A;
    auto R = A * I;
    RT_ASSERT_TRUE(L == A && R == A);
}

RT_TEST(SMatrix_transform_vector){
    SMatrix<3,3,float> A{1,2,3, 4,5,6, 7,8,9};
    Vec3f v{1.f, 0.f, -1.f};
    auto out = A * v; // uses operator*(Vector)
    // Row-major multiply check
    RT_ASSERT_TRUE(std::fabs(out[0] - (1*1 + 2*0 + 3*(-1))) < 1e-6f);
    RT_ASSERT_TRUE(std::fabs(out[1] - (4*1 + 5*0 + 6*(-1))) < 1e-6f);
    RT_ASSERT_TRUE(std::fabs(out[2] - (7*1 + 8*0 + 9*(-1))) < 1e-6f);
}

RT_TEST(SMatrix_is_zero_identity){
