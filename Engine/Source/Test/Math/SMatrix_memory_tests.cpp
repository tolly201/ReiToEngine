...
RT_TEST(SMatrix_copy_move_and_assign){
    SMatrix<2,2,int> A{1,2,3,4};
    SMatrix<2,2,int> B{A}; // copy
    RT_ASSERT_TRUE(B.data[0]==1 && B.data[3]==4);

    SMatrix<2,2,int> C;
    C = A; // assign
    RT_ASSERT_TRUE(C.data[1]==2 && C.data[2]==3);

    SMatrix<2,2,int> D{std::move(A)}; // move should preserve values due to array copy
    RT_ASSERT_TRUE(D.data[0]==1 && D.data[3]==4);
}

RT_TEST(SMatrix_new_delete){
    auto* p = new SMatrix<3,3,float>{1,0,0, 0,1,0, 0,0,1};
    RT_ASSERT_TRUE(p->IsIdentity());
    auto addr = reinterpret_cast<std::uintptr_t>(p);
    RT_ASSERT_TRUE(addr % alignof(SMatrix<3,3,float>) == 0);
    delete p;

    auto* arr = new SMatrix<2,2,int>[3];
    arr[2] = SMatrix<2,2,int>{1,2,3,4};
    RT_ASSERT_TRUE(arr[2].data[0]==1 && arr[2].data[3]==4);
    delete[] arr;
}

RT_TEST(SMatrix_determinant_and_rank){
    // 2x2 det
    SMatrix<2,2,int> M2{1,2,3,4};
    RT_ASSERT_TRUE(M2.Determinant() == (1*4 - 2*3));
